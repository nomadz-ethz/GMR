> **Archived upstream README.** This is the README from the upstream
> [`YanjieZe/GMR`](https://github.com/YanjieZe/GMR) project, kept here for
> historical reference. Some asset references (notably `optitrack.png` and
> the per-robot demos for robots removed in this fork) may not resolve.
> See [`../overview.md`](../overview.md) for the current fork's introduction.
